...
<div class="leading-loose flex justify-center">
  <form class="w-75  p-10 bg-white rounded shadow-xl">
    ...
    <div class="">
    {!! Form::open(['url' => route('poste.store'),'method' => 'post']) !!}
      <label class="block text-sm text-gray-00" for="cus_name">Nom de l'entreprise</label>
      <input class="w-full px-3 py-1 text-gray-700 bg-gray-200 rounded" id="cus_name" name="nomEntreprise" type="text" required="" placeholder="Nom">
    </div>
    <div class="mt-2">
    ...
    @foreach($categorie as $ligne)
        <div class="mt-2 space-y-4">
            <div class="flex items-center">
                <input id="{{$ligne->id}}" name="categorie" type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
    ...
    @foreach($tab as $ligne)
        <div class="mt-2 space-y-4">
            <div class="flex items-center">
                <input id="{{$ligne->id}}" name="type" type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
                <label for="" class="ml-3 block text-sm font-medium text-gray-700">{{$ligne->libelle}}</label>
            </div>
        </div>
    @endforeach
